    ...
    public partial class  comday : Page
    {
        ...
        {
            string sql = "select CardType,sum(case DATEDIFF(day,Addtime,'" + this.day + "') when 0 then CONVERT(decimal(18, 2),[money]) else 0.00 end) as '今天',sum(case DATEDIFF(day,Addtime,'" + this.day + "') when 1 then CONVERT(decimal(18, 2),[money]) else 0.00 end) as '昨天' from Consumption_Back_select group by CardType ";
